...
departures.forEach((row) => {
  ...
  btnDelay.addEventListener('click', () => {
    ...
    if (clicked) {
      rowElm.classList.add('row-delayed');
      row['status'] = 'Delay';
      rowElm.querySelector('.board__status').textContent = 'Delay';
    } else {
      rowElm.classList.remove('row-delayed');
      row['status'] = 'On Time';
      rowElm.querySelector('.board__status').textContent = 'On Time';
    }
  });
